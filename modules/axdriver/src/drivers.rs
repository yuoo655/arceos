...
cfg_if::cfg_if! {
    if #[cfg(net_dev = "bstnic")] {
        use super::BstNicTraitsImpl;
        pub struct BstNicDriver;
        register_net_driver!(BstNicDriver, driver_net::bst::BstNic<BstNicTraitsImpl>);

        impl DriverProbe for BstNicDriver {
            fn probe_global() -> Option<AxDeviceEnum> {
                use driver_net::bst::BstNic;

                let bst_nic = BstNic::init(BstNicTraitsImpl);
                return Some(AxDeviceEnum::from_net(bst_nic));
            }
        }
    }
}
